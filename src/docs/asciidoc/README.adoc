...
<2> *Enable/Disable* tracing at the application level.  If disabled, no traces will be generated or exported.
<3> Provide a *service name* - usually the application name.
<4> Configure *Collector Endpoint* for trace data - must be the entire URL, including the scheme (HTTP/S) and port.
<5> Configure OTLP *Transport Protocol*  - the following transports are supported:  `GRPC`, `HTTP_JSON` and `HTTP_PROTOBUF`.
<6> Various configuration parameters to control trace/span batch export rate.
<7> Add any necessary vendor-specific headers (e.g., `Authorization` header with `API Token` key for authentication)
<8> Optionally disable generating span data for all Message Processors - default behavior is generate span data for all
Message Processors.
<9> Or mute individual Message Processor(s) from generating span data (this maybe helpful in eliminating "noise" from the trace
and let you more effectively focus in on Message Processors(s) of concern).

==== Agent Configuration with certificate

This new Mule custom module of Open Telemetry also supports the OLTP endpoint with a self-signed certificate by adding Truststore into the configuration. The truststore file should contain one or more X.509 certificates in PEM format. Below are the steps for configuring it:

image::agent-connector-config-with-certificate.png[800, 800, title="Mule Agent Configuration with certificate", align="center"]


<1> Put the location of the Truststore file from Mule Application Classpath.
<2> Truststore file (X.509 certificates in PEM format) location that will load to Mule Application Classpath.
