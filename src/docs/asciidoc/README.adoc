...
// Document header
Rick Bansal <rickbansal@salesforce.com>
:revnumber:
:revdate:
:revremark: Published
:doctype: book
...
<1> The version could be different based upon when you read this document and which version was downloaded.  Please make sure the version 
property corresponds to the version you downloaded.
...
----
<1> The version could be different based upon when you read this document and which version was downloaded.  Please make sure the version 
element corresponds to the version you installed into your Maven repository.  The latest version is `otel-mule4-observability-agent-{revnumber}-mule-plugin.jar`

=== Agent Configuration

Minimally, follow the steps below to add and configure the agent into your Mule application.

IMPORTANT: Mule applications *must add the agent/extension* to their configuration in order to generate and *export* trace and/or metrics telemetry.

==== General Configuration
image::agent-connector-config.png[1000, 1000, title="Mule Agent Configuration - General", align="center"]
====
<1> *Add* an OpenTelemetry Mule 4 Observability Configuration to the Mule project.
<2> *Enable/Disable* telemetry generation at the application level.  If disabled, no signals (e.g., traces, metrics, ...) will be generated or exported.
<3> Provide a *service name* - usually the application name.
<4> Optionally provide application level *resource attributes* as `<key, value>` pairs
====

==== Metrics Configuration
[NOTE]
====
Metrics generation is in an *_"experimental"_* stage and currently, only the following metrics are generated and exported:

* Heap usage:
+
The amount of heap memory (in megabytes) being used when the flow starts
* Workload utilization:
+
Calculated by `[workload average/total CPUs] * 100%` where the _workload average_ is the sum of the number of runnable entities queued to 
the available processors and the number of runnable entities running on the available processors averaged over the last minute of time. 
====
image::agent-connector-config-metrics.png[800, 800, title="Mule Agent Configuration - Metrics", align="center"]
====
<1> *Enable/Disable* metrics generation at the application level.  If disabled (default), no metrics signals will be generated or exported.
<2> Configure *Collector Endpoint* for metrics data - must be the entire URL, including the scheme (HTTP/S) and port.
<3> Configure OTLP *Transport Protocol*  - the following transports are supported:  `GRPC`, `HTTP_JSON` and `HTTP_PROTOBUF`.
<4> Add any necessary vendor-specific *headers* (e.g., `Authorization` header with `API Token` key for authentication)
<5> Optionally set metrics data *export interval* - default is `60000 milliseconds`.
====

==== Traces Configuration
image::agent-connector-config-traces.png[800, 800, title="Mule Agent Configuration - Traces", align="center"]
====
<1> *Enable/Disable* tracing at the application level.  If disabled, no trace signals will be generated or exported.
<2> Configure *Collector Endpoint* for trace data - must be the entire URL, including the scheme (HTTP/S) and port.
<3> Configure OTLP *Transport Protocol*  - the following transports are supported:  `GRPC`, `HTTP_JSON` and `HTTP_PROTOBUF`.
<4> Add any necessary vendor-specific *headers* (e.g., `Authorization` header with `API Token` key for authentication)
<5> Various configuration parameters to control trace/span batch *export rate*.
...
<8> Select whether or not to export *custom attributes* to each flow span, each message processor span, neither or both - default 
behavior is to not export custom attributes for flows or message processors.
<9> Define *custom attributes* as `<key, value>` pairs. Either the `key` or `value` can be defined by a DataWeave expression.

Note:  If using DataWeave expressions, put quotes (`""`) around the expression (e.g., `"#[vars.orderId]"`.
...
==== Context Extraction
The agent will automatically *extract* the trace headers from an incoming HTTP request or a message received on an Anypoint MQ queue and 
*inject* the headers into the application via an event *variable* named: `OTEL_TRACE_CONTEXT` of type:  `Map<String, String>`, 
where `Map<String, String>` contains the following:
...
==== Anypoint MQ Context Propagation
In order to *propagate the trace header* information to other Anypoint MQ based applications, the Mule Anypoint Publisher 
*must* have User Properties configured in the following way:

image::amq-publisher-userprops.png[800, 800, title="Mule Anypoint MQ Publisher User Properties", align="center"]

.Mule configuration xml for setting User Properties in the Anypoint MQ Publisher
[source%nowrap%linenums, xml]
----
<anypoint-mq:publish doc:name="Publish" doc:id="50a6b30f-e1a0-44a9-b885-55d005233636" config-ref="Anypoint_MQ_Config" destination="mq-demo-q1-fifo">
    <anypoint-mq:properties>
        <![CDATA[#[%dw 2.0
          output application/java
          ---
          {
	         traceparent : vars.OTEL_TRACE_CONTEXT.traceparent default '' as String, <1>
	         tracestate : vars.OTEL_TRACE_CONTEXT.tracestate default '' as String <2>
          }]]]>
    </anypoint-mq:properties>
</anypoint-mq:publish>
----
...
Below is a description of the demo scenario used to generate distributed trace information from *3 Mule applications* and have it
...
<1> External application sending a request to Mule App_1 with WC3 Trace Context Headers
<2> Mule App 1 sending a request to Mule App_2 and propagating the trace context via WC3 Trace Context Headers
<3> Response coming back to calling application
<4> Mule App_1 sending a message to Mule App_3 via Anypoint MQ
<5> Response coming back to external calling application
<6> Mule App_3 *asynchronously* processing incoming message from Mule App_1 via Anypoint MQ
<7> All Mule applications sending log and metrics data to Anypoint Monitoring
<8> All Mule 4 OTel Agent sending trace/metrics information to Dynatrace OTel Collector
<9> Dynatrace Collector forwarding the data to Dynatrace dashboard for rendering

==== Demo Architecture Mule Implementation Diagram

Below is an *implementation* of the demo architecture described above. At a high-level:

<1> *Mule_App_1* receives the initial request from the *external client*, performs various functions including making a 
<2> Synchronous request to external application, *Mule_App_2* via HTTP, and making an
<3> Asynchronous request to external application *Mule_App_3* via Anypoint MQ

Other sub-flows are executed within Mule_App_1 before returning a response to the calling client application.
...
<3> Represents the overall set of *spans* associated with the *external Mule application (Mule_App_3)*. Nested (child) spans are indented 
appropriately.
...
and if a Message Processor span then Message Processor type (e.g. Logger, Transform, DB, HTTP Requester, MQ Publisher, ...).
** For root spans, there two interesting *metric attributes* which are generated:
*** `mule.start.heap.usage.mbytes`: 
+
The amount of heap memory (in megabytes) being used when the flow starts
*** `mule.start.workload.utilization`:
+
`= [workload average/total CPUs] * 100%` when the flow starts.
+
The _workload average_ is the sum of the
number of runnable entities queued to the available processors and the number of runnable entities running on the available processors 
averaged over the last minute of time. The system load average indicates not only CPU demand but also file I/O demand, network I/O 
demand, disk I/0 demand, and cycles waiting for locks.

[NOTE]
.Example:
====
Assume the *workload average is 6* over the last minute, then for:

* CPU = 1,
+
`mule.start.workload.utilization = workloadAverage/#CPUs = 6/1 * 100% = 600%`
+
which means the system was *over-utilized by 500%*, that is, the system had 5x more work queued up then the system could handle
over the past minute.

* CPU = 12,
+
`mule.start.workload.utilization = workloadAverage/#CPUs = 6/12 * 100% = 50%`
+
which means the system was *under-utilized by 50%*, that is, the system could've handled an additional 2x more work over 
the past minute.
====
...
==== Anypoint MQ Publisher/Subscriber Processor Summary

Below is a diagram which depicts specific attributes for both the Anypoint MQ Publish and Subscribe Message Processors.  The extension 
will generate connection related attributes such as the event location (i.e., Publish/Subscribe queue/topic), payload type and message id.

image::dynatrace-distributed-mqSub-mqPub-summary.png[title="Dynatrace - Anypoint Publisher/Subscriber Processor Summary", align="center"]
