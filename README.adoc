...
Trace Root Span:: A Mule root span is an OTel Span which serves as the root node in a Mule trace.  It is associated
with the initial Mule Flow. In reality it is also a pipeline span.

Pipeline Span:: A Mule pipeline span is an OTel Span which is associated with subflows and/or flow references.

Message Processor Span:: A Mule
...
* *Start* an OTel Span - known as the *root parent span*
* Add flow input metadata to the root parent span

====== MessageProcessorNotification.MESSAGE_PROCESSOR_PRE_INVOKE

* *Start* an OTel Span - known as a *child span*
* Add message processor input metadata to the child span
* Add the child span to the root parent span

====== MessageProcessorNotification.MESSAGE_PROCESSOR_POST_INVOKE

* Add message processor output metadata to the child span
* *End* the *child span*

====== PipelineMessageNotification.PROCESS_END

* Add flow output metadata to the root parent span
* End all child spans
* End the root parent span
* Export the trace
