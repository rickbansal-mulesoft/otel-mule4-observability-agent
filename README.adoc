...
////
* Instantiates and configures the OTel Java SDK
* Registers the following Mule Server notification listeners:
** `PipelineMessageNotificationListener`
** `MessageProcessorNotificationListener`
* Listens for the subscribed notifications
////
...
////
* If associated with the *initial flow*:
** Start a Mule trace
** Start a Trace Root Span 
** Add Trace Root Span to the Trace
* If *subflow* or *flow reference*:
** Start a Pipeline Span
** Add Pipeline Span to the Trace
* Add flow input metadata to started span

====== MessageProcessorNotification.MESSAGE_PROCESSOR_PRE_INVOKE

* Start a Message Processor Span
* Add message processor input metadata to the span
* Add the span to the current Pipeline span

====== MessageProcessorNotification.MESSAGE_PROCESSOR_POST_INVOKE

* Add message processor output metadata to the open Message Processor Span
* End the open Message Processor Span

====== PipelineMessageNotification.PROCESS_END

* If associated with the *initial flow*:
** Add flow output metadata to the Trace Root Span 
** End Trace
*** End all Pipeline Spans
**** End all Message Processor Spans
** Export the trace
* If *subflow* or *flow reference*:
** End Pipeline Span
*** End all Message Processor Spans in the Pipeline Span
////
...
<1> Add a OpenTelemetry Mule 4 Observability Configuration to the Mule project
